...
for i in range(dim[0]/pixelCalcWidth):
	...
	for j in range(dim[1]/pixelCalcWidth):  
		...
		T[i,j] = ret.x
		D[i,j] = doseMedian(T[i,j],OD,w,red,green,blue,pixelCalcWidth)
...
fig4 = figure(4)
clf()
pic1 = imshow(D, cmap=cm.jet, vmin=D.min(), vmax=D.max(), interpolation='none', extent=[y.min(),y.max(),x.max(),x.min()])
fig4.colorbar(pic1)
